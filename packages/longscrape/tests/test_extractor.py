...
def partial_html_doc() -> Document:
    return Document(
        ...
        content_type="text/html",
        content="<html>   <body>       <h1>Hello world!</h1>   </body></html>".encode(
            "utf-8"
        ),
    )
...
class ListItemsExtractor(Extractor[dict[str, Any]]):
    """Yields 0..N records from repeated tags."""

    async def extract(
        self, document: Document
    ) -> AsyncIterable[Record[dict[str, Any]]]:
...
class LenientExampleExtractor(Extractor[ExamplePartialRecord]):
    ...
    async def extract(
        self, document: Document
    ) -> AsyncIterable[Record[ExamplePartialRecord]]:
        h1_match = re.search(r"<h1>(.*?)</h1>", document.text)
        h2_match = re.search(r"<h2>(.*?)</h2>", document.text)
...
async def test_multiple_records_extract():
    list_doc = Document(
        ...
        content_type="text/html",
        content="<ul><li>Item 1</li><li>Item 2</li><li>Item 3</li></ul>".encode(
            "utf-8"
        ),
    )
...
async def test_zero_records_extract_returns_empty_list():
    empty_doc = Document(
        ...
        content_type="text/html",
        content="<html><body><p>No target elements here</p></body></html>".encode(
            "utf-8"
        ),
    )
    extractor = SingleRecordExtractor()
...
@pytest.mark.asyncio
async def test_strict_extractor_fails_on_missing_required_data(
    partial_html_doc: Document,
):
...
@pytest.mark.asyncio
async def test_lenient_extractor_returns_none_on_missing_optional_data(
    partial_html_doc: Document,
):
